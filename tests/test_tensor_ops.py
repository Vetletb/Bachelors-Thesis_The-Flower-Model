...
def test_cosine_similarity_returns_correct_value(f, i, p):
    ...
    real_expected = torch.nn.functional.cosine_similarity(a[f], b[i,:,p]*a_abs[f], dim=0)
    imag_expected = torch.nn.functional.cosine_similarity(a[f+2], b[i,:,p]*a_abs[f], dim=0)
